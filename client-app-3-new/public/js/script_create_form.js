...
myApp.controller('AdminController', ['$scope', '$window', '$http', function ($scope, $window, $http) {
    ...
    $scope.allSelected = false;
 	$scope.subjects1=['Data Structure','Networking','Automata','Microprocessor','Operating system','Database Management System','Computer Organization and Architecture','Compiler','Algorithm','Software Engineering','Object oriented Technology'];
 	 $scope.subjects2=['DS','DBMS'];
     $scope.subjects3=['Analog Electronics','Digital Electronics'];
     $scope.subjects4=['Microbiology','Biocemical analysis & Instrumentation','Structure of Macromolecules','Biochemistry','Cell Biology','Molecular Biology','Immunology','Thermodynamics','Industrial Microbiology','Unit Operations of Chemical Engineering II','Bioreactor Design and analysis','Genetics and Biostatistics','Process control and Instrumentation','DBMS','Plant Biotechnology','Bioinformatics','DE I','DE II','DE III','DE IV'];
     $scope.subjects5=['Solid Mechanics','Fluid Mechanics'];
     $scope.subjects6=['Thermodynamics'];
     $scope.subjects7=['Solid Mechanics'];
     $scope.subjects8=['Iron Making'];
     $scope.subjects9=['Machine Design'];
...
//subject selection
 /*$scope.subjectSelectionDone = function (done) {
        if (done) {
            // get the values of selected depts
            // push it into $scope.selectedDepartments

            $scope.subjects.forEach(function (subject) {
                var input_Element = document.getElementById('subject-' + subject);
                if (input_Element.checked) {
                    // if already not in array push


                    if ($scope.inArray($scope.selectedSubjects, input_Element.value) == -1) {
                        $scope.selectedSubjects.push(input_Element.value);
                    }
                } else {
                    // not checked means u need to remove
                    if ($scope.inArray($scope.selectedSubjects, input_Element.value) != -1) {
                        var foundIndex = $scope.inArray($scope.selectedSubjects, input_Element.value);
                        $scope.selectedSubjects.splice(foundIndex, 1);
                    }
                }
            });
            ...
            // now disable the inputs
            $scope.subjects.forEach(function (subject) {
                document.getElementById('subject-' + subject).disabled = true;
            });

        } else {
            $scope.subjects.forEach(function (subject) {
                document.getElementById('subject-' + subject).disabled = false;
            });
        }

        console.log("Subject data:", $scope.selectedSubjects);
    }; */
    ...
    $scope.submitForm = function () {
        ...
        $scope.finalObject.targetedsub = $scope.selectedSubjects;
